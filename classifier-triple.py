...
clf = linear_model.Perceptron()#tree.DecisionTreeClassifier()
titles = []
weights = []
...
def generateTrainDataPoints(tpl):
    ...
    if(I['year'] < J['year'] < K['year']):
        ...
        b = -1
    val = ({'title1':I['title'],'sentences1':I['title'],\
            'title2':J['title'],'sentences2': J['title'],\
            'title3':K['title'],'sentences3': K['title'],\
...
tripleSets = p.map(generateTrainDataPoints,mapping[0:15])
for i in tripleSets:
...
print datetime.datetime.now()
doubleSets = p.map(generateTestDataPoints,mapping[0:7])
for i in doubleSets:
    if(i['year'] == -1):
        doubleSets.remove(i)
print datetime.datetime.now()
