...
def pagerank_scipy(G, alpha=0.85, personalization=None,
    ...
    print("Creating nodelist...")
    nodelist = G.nodes()

    print("Converting nodelist to scipy matrix...")
    M = nx.to_scipy_sparse_matrix(G, nodelist=nodelist, weight=weight,
                                  dtype=float)

    print("Calculating things I don't understand...")
    ...
    print("Calculating dangling nodes...")
    ...
    for y in range(max_iter):
        print("Iteration No. %s..." % y)
        xlast = x
        x = alpha * (x * M + sum(x[is_dangling]) * dangling_weights) + \
            (1 - alpha) * p
        # check convergence, l1 norm
        err = scipy.absolute(x - xlast).sum()
        print("Got an error of %s" % err)
        if err < tol:  # Why N * tol???
            return dict(zip(nodelist, map(float, x)))

    if y == max_iter - 1:
        print('pagerank_scipy: power iteration failed to converge in %d iterations.' % max_iter)
        print('current error is %s' % err)
        return dict(zip(nodelist, map(float, x)))
