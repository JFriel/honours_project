...
def generateTrainDataPoints(tpl):
    ...
    val = ({'title1':I['title'],'title1':I['title'],\
            ...
            'year':b, 'vocab':set(titleI + titleJ)})
    return val
...
def getFeature(item):
    yr = item['year']
    vec = fe.get(item['title1'],item['title2'])
    titles = ([item['title1'],item['title2']])
    return ([vec,titles,yr])
#C
def train(features):

    Y = [item[2] for item in features if len(item[0]) == 15]
    X = [item[0] for item in features if len(item[0]) == 15]
    clf.fit(X,Y)
