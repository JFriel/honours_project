...
\cite{hofmann2008kernel}
 ...
 L1, also know as  the least absolute deviatinears, is `a loss function that minimises the sum of of the absolute
 ...
 Duriing our experimentation with Perceptron paramaters, we dicovered that giving the system nine epochs (passes
 over the training data) and shuffling the training data on each itteration provided results that maximised the
 estimations. For the same reasons stated in section \ref{sec:log-reg}, we use an L2 penelty with our perceptron
 model.
 ...
 The last parameter we has to exeriment with was the number of epochs.
 We wanted each layer to reach a convergence, so the paramater was set to 100.
 100 layers was chosen as during experimentation with subsets of the data improvments were
