...
\begin{itemize}
\item Scikit-learning~\cite{} - Machine Learning tools for Python
\item Numpy~\cite{} - package for scientific computation
\item Scipy~\cite{} - Python library for scientific tools
\item Wikipedia~\cite{} - Python wrapper for integrating with Wikipedia's API
\item Pool~\cite{} - Multiprocessing utility for Python
\item Pattern.en~\cite{} - A part-of-speech tagger 
\item Networkx~\cite{} - Graph utility for Python
\item Matplotlib~\cite{} - Visualisation tools
\end{itemize}
...
Global Inference is producting the best global solution by inferring predictiond from each input~\cite{roth2004linear}.%More here
